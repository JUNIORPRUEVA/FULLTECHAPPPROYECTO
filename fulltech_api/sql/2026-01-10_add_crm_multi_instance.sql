...
BEGIN
  ...
  SELECT EXISTS (
    ...
    WHERE table_name = 'crm_chats' AND column_name = 'assigned_user_id'
  ) INTO has_assigned_user_id;
  ...
  IF EXISTS (SELECT 1 FROM crm_chats LIMIT 1) AND NOT EXISTS (SELECT 1 FROM crm_instancias LIMIT 1) THEN
    ...
    IF has_assigned_user_id THEN
      EXECUTE format(
        'UPDATE crm_chats SET instancia_id = $1, owner_user_id = $2, asignado_a_user_id = COALESCE(assigned_user_id, $2) WHERE instancia_id IS NULL'
      ) USING default_instance_id, default_user_id;
    ELSE
      UPDATE crm_chats
      SET
        instancia_id = default_instance_id,
        owner_user_id = default_user_id,
        asignado_a_user_id = default_user_id
...
DROP TRIGGER IF EXISTS crm_instancias_updated_at_trigger ON crm_instancias;
