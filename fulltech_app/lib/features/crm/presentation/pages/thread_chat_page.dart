...
import '../widgets/quick_replies_modal.dart';
...
class ThreadChatPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ModulePage(
      ...
      child: Column(
        children: [
          Flexible(flex: 3, child: ThreadChatPanel(threadId: threadId)),
          const SizedBox(height: 12),
          Flexible(flex: 2, child: RightPanelCrm(threadId: threadId)),
        ],
      ),
    );
  }
...
class _ThreadChatPanelState extends ConsumerState<ThreadChatPanel> {
  ...
  void initState() {
    ...
    Future.microtask(() async {
      if (kDebugMode) {
        debugPrint(
          '[CRM][UI] ThreadChatPanel initState threadId=${widget.threadId}',
        );
      }
      await ref
  ...
  @override
  void didUpdateWidget(covariant ThreadChatPanel oldWidget) {
    ...
    Future.microtask(() async {
      if (kDebugMode) {
        debugPrint(
          '[CRM][UI] ThreadChatPanel threadId changed ${oldWidget.threadId} -> ${widget.threadId}',
        );
      }
  ...
  Future<void> _send(dynamic notifier) async {
  ...
  static String _resolveTemplate(
    ...
    // Empresa: no disponible en este contexto; dejar vacío.
  ...
  void _insertIntoInput(String text) {
    ...
    final next = base.trim().isEmpty ? insert : '$base\n$insert';
    _textCtrl.value = v.copyWith(
      text: next,
