    ...
    for (final item in items) {
      ...
      try {
        if (item.op == 'upsert') {
          ...
          final empresaId = (remote['empresa_id'] ?? remote['empresaId'] ?? '').toString();

          // Best-effort: update local header.
          final existing = await _db.getCotizacion(id: item.entityId);
          final base = existing == null ? <String, Object?>{} : <String, Object?>{...existing};
          base['id'] = remoteId;
          if (empresaId.isNotEmpty) base['empresa_id'] = empresaId;
          base['numero'] = (remote['numero'] ?? base['numero'] ?? '').toString();
          base['status'] = (remote['status'] ?? base['status'] ?? 'draft').toString();
          base['customer_id'] = (remote['customerId'] ?? remote['customer_id'] ?? base['customer_id']);
          base['customer_name'] = (remote['customerName'] ?? remote['customer_name'] ?? base['customer_name']);
          base['customer_phone'] = (remote['customerPhone'] ?? remote['customer_phone'] ?? base['customer_phone']);
          base['customer_email'] = (remote['customerEmail'] ?? remote['customer_email'] ?? base['customer_email']);
          base['itbis_enabled'] = ((remote['itbisEnabled'] ?? remote['itbis_enabled']) == true) ? 1 : (base['itbis_enabled'] ?? 0);
          base['itbis_rate'] = (remote['itbisRate'] ?? remote['itbis_rate'] ?? base['itbis_rate']);
          base['subtotal'] = (remote['subtotal'] ?? base['subtotal']);
          base['itbis_amount'] = (remote['itbisAmount'] ?? remote['itbis_amount'] ?? base['itbis_amount']);
          base['total'] = (remote['total'] ?? base['total']);
          base['notes'] = (remote['notes'] ?? base['notes']);
          base['created_at'] = (remote['createdAt'] ?? remote['created_at'] ?? base['created_at'] ?? DateTime.now().toIso8601String()).toString();
          base['updated_at'] = (remote['updatedAt'] ?? remote['updated_at'] ?? DateTime.now().toIso8601String()).toString();
          ...
          final remoteItems = (remote['items'] as List?)?.cast<Map<String, dynamic>>() ?? const <Map<String, dynamic>>[];
          if (remoteItems.isNotEmpty) {
            await _db.replaceCotizacionItems(
              quotationId: remoteId,
              items: remoteItems
                  .map((it) => _mapServerItemToLocal(it, cotizacionId: remoteId))
